...
public class ContainerRepair extends net.minecraft.inventory.ContainerRepair {
     ...
     */
    private IInventory inputSlots = new mod.cvbox.container.InventoryRepair(this, "Repair", true, 2);
    ...
    public ContainerRepair(InventoryPlayer inventoryPlayer, World world, int x, int y, int z, EntityPlayer entityPlayer) {
        ...
        this.addSlotToContainer(new mod.cvbox.container.SlotRepair(this, this.outputSlot, 2, 134, 47, world, x, y, z));
        int l;
    ...
    public void onCraftMatrixChanged(IInventory iInventory) {
        ...
        if (iInventory == this.inputSlots) {
            if(ConfigValue.General.legacyMode) {
    ...
    public void updateRepairOutput() {
        ...
        if(stack1 == null) {
                ...
                if(stack1.getItem() == stack2.getItem()) {
                    ...
                    if(!enchantments2.isEmpty()) {
                        ...
                        for(Map.Entry<Enchantment, Integer> entry: enchantments2.entrySet()) {
                            repairCost += entry.getValue() * ConfigValue.General.copyEnchantToBookCostMultiplier;
                            repairAmount += ConfigValue.General.copyEnchantToBookRepairBonus;
                        }
                    ...
                    if(it.hasNext()) {
                        ...
                        repairCost += ench.getValue() * ConfigValue.General.copyEnchantToBookCostMultiplier;
                        repairAmount += ConfigValue.General.copyEnchantToBookRepairBonus;
            ...
            if (this.repairedItemName != null && this.repairedItemName.length() > 0 && !this.repairedItemName.equals(stack1.getDisplayName())) {
                ...
                repairCost += ConfigValue.General.renameingCost;
                this.isRenamingOnly = repairCost == ConfigValue.General.renameingCost;
                if (stack1.getItem().isRepairable()) repairAmount += ConfigValue.General.renamingRepairBonus;
            }
            //Repair
            if(stack2 != null && stack1.getItem() == stack2.getItem() && stack1.getItem().isRepairable()) {
                double amount = stack2.getMaxDamage() - stack2.getItemDamage() + ((double)stack1.getMaxDamage() * ConfigValue.General.mainRepairBonusPercent);
                repairAmount += amount;
                ...
                for(int i = 0; i < stack2.stackSize && damage < max; i++) {
                    damage = Math.min(damage + (max * ConfigValue.General.itemRepairAmount), max);
                    amount++;
                }
                this.resultInputStack = stack2.copy();
                ...
                repairCost += amount * ConfigValue.General.repairCostPreItem;
            }
            //Set outputs
            workStack.setItemDamage((int)Math.round(workStack.getItemDamage() - repairAmount));
            this.maximumCost = (int) Math.round(repairCost * ConfigValue.General.constMultiplaier);
	...
	public void updateRepairOutputOld() {
        ...
        if (itemstack == null) {
            ...
            Map<Enchantment, Integer> enchantmentMap = (Map<Enchantment, Integer>)EnchantmentHelper.getEnchantments(itemstack1);
            boolean flag = false;
            ...
            if (itemstack2 != null) {
                flag = itemstack2.getItem() == Items.enchanted_book && Items.enchanted_book.getEnchantments(itemstack2).tagCount() > 0;//has enchantments on book

                if (itemstack1.isItemStackDamageable() && itemstack1.getItem().getIsRepairable(itemstack, itemstack2)) {
                    itemDamage1 = Math.min(itemstack1.getItemDamage(), itemstack1.getMaxDamage() / 4);
                    ...
                    for (tempInt = 0; itemDamage1 > 0 && tempInt < itemstack2.stackSize; ++tempInt) {
                        tempInt1 = itemstack1.getItemDamage() - itemDamage1;
                    ...
                    if (itemstack1.isItemStackDamageable() && !flag) {
                        itemDamage1 = itemstack.getMaxDamage() - itemstack.getItemDamage();
                        tempInt = itemstack2.getMaxDamage() - itemstack2.getItemDamage();
                    ...
                    Map<Enchantment, Integer> enchantmentMap1 = (Map<Enchantment, Integer>)EnchantmentHelper.getEnchantments(itemstack2);
                    iterator = enchantmentMap1.keySet().iterator();

                    while (iterator.hasNext()) {
//                        tempInt1 = iterator.next();
//                        enchantment = Enchantment.enchantmentsBookList[tempInt1];
                        enchantment = iterator.next();
                        tempInt1 = Enchantment.getEnchantmentID(enchantment);
                        ...
                        for(Enchantment enchantmentValue1 : enchantmentMap.keySet()) {
                            if (Enchantment.getEnchantmentID(enchantmentValue1) != tempInt1 &&
                            		!Utils.canApplyTogether(enchantment, enchantmentValue1)) {
                                flag1 = false;
                        ...
                        if (flag1) {
                            ...
                            enchantmentMap.put(enchantment, tempInt3);
                            int enchantmentWeight = 0;
                            switch (enchantment.getWeight().getWeight()) {
                                case 1:
                                    enchantmentWeight = 8;
                                    break;
                                case 2:
...
//                enchantment = Enchantment.enchantmentsBookList[tempInt1];
            ...
            this.maximumCost = (int)Math.round((repairCost + itemDamage) * ConfigValue.General.constMultiplaier);
            if (itemDamage <= 0) {
                itemstack1 = null;
            }
            if (repairAmount == itemDamage && repairAmount > 0) {
                if(ConfigValue.General.renameingCost == 0) {
    ...
    public static IInventory getRepairInputInventory(mod.cvbox.container.ContainerRepair containerRepairBA) {
        return containerRepairBA.inputSlots;
    }

    public static int getStackSizeUsedInRepair(mod.cvbox.container.ContainerRepair containerRepairBA) {
