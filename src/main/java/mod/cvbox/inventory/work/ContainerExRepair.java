...
public class ContainerExRepair extends Container {
	...
	public ContainerExRepair(int id, PlayerInventory playerInv) {
		this(id, playerInv, IWorldPosCallable.DUMMY);
	}

	public ContainerExRepair(int id, PlayerInventory playerInv, final IWorldPosCallable pos) {
		super(Mod_ConvenienceBox.CONTAINER_EXREPAIR, id);
		this.position = pos;
		...
		this.addSlot(new Slot(this.outputSlot, 2, 134, 47) {
			...
			*/
			public boolean canTakeStack(PlayerEntity playerIn) {
				return (playerIn.abilities.isCreativeMode || playerIn.experienceLevel >= ContainerExRepair.this.maximumCost.get()) && ContainerExRepair.this.maximumCost.get() > 0 && this.getHasStack();
			}

			public ItemStack onTake(PlayerEntity thePlayer, ItemStack stack) {
				if (!thePlayer.abilities.isCreativeMode) {
					thePlayer.addExperienceLevel(-ContainerExRepair.this.maximumCost.get());
				}

				float breakChance = net.minecraftforge.common.ForgeHooks.onAnvilRepair(thePlayer, stack, ContainerExRepair.this.inputSlots.getStackInSlot(0), ContainerExRepair.this.inputSlots.getStackInSlot(1));
				...
				ContainerExRepair.this.maximumCost.set(0);
				return stack;
			}
		});
	...
	*/
	public boolean canInteractWith(PlayerEntity playerIn) {
		return this.position.applyOrElse((world, pos) -> {
			return !(world.getBlockState(pos).getBlock()==BlockCore.block_exanvil) ? false : playerIn.getDistanceSq((double)pos.getX() + 0.5D, (double)pos.getY() + 0.5D, (double)pos.getZ() + 0.5D) <= 64.0D;
		}, true);
	}
