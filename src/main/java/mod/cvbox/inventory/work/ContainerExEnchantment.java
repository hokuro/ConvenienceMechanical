...
import mod.cvbox.inventory.InventoryBasicEx;
...
public class ContainerExEnchantment extends Container {
    ...
    public ContainerExEnchantment(int id, PlayerInventory playerInv, BlockPos pos){
		super(Mod_ConvenienceBox.CONTAINER_EXENCHANTMENT, id);
        player = playerInv.player;
        worldPointer = player.world;
        position = pos;
    	...
    	if (!worldPointer.isRemote){
            if (inventoryIn == this.tableInventory) {
            	if (ModUtil.compareItemStacks(before_sucrifice, this.tableInventory.getStackInSlot(1), CompaierLevel.LEVEL_EQUAL_ALL)){
    				MessageHandler.SendMessage_ExEnchant_ClearParameter((ServerPlayerEntity) player);
            	}
	...
	protected Enchantment[] readEnchant(ItemStack stack){
		ListNBT encs = stack.getEnchantmentTagList();
		Enchantment[] ret = new Enchantment[encs.size()];

		for (int i = 0; i < ret.length; i++){
			CompoundNBT nbt = encs.getCompound(i);
    ...
    public boolean enchantItem(PlayerEntity playerIn) {
        ...
        if ((itemstack1.isEmpty() || itemstack1.getCount() < itemCnt) && !playerIn.abilities.isCreativeMode) {
            ...
            if (!this.worldPointer.isRemote) {
                ...
                if (flag) {
                    EnchantedBookItem.addEnchantment(itemstack, new EnchantmentData(enc,enc_level));
                } else {
                ...
                if (playerIn instanceof ServerPlayerEntity) {
                    CriteriaTriggers.ENCHANTED_ITEM.trigger((ServerPlayerEntity)playerIn, itemstack, itemCnt);
                }

                this.tableInventory.markDirty();
                this.onCraftMatrixChanged(this.tableInventory);
                this.worldPointer.playSound((PlayerEntity)null, this.position, SoundEvents.BLOCK_ENCHANTMENT_TABLE_USE, SoundCategory.BLOCKS, 1.0F, this.worldPointer.rand.nextFloat() * 0.1F + 0.9F);
            }
            return true;
	...
	public static void updateEnchantment(int index, int level, PlayerEntity player) {
	...
	public static void execEnchant(PlayerEntity player) {
		if (player.openContainer instanceof ContainerExEnchantment){
