...
public class ContainerWoodPlanter extends Container {
	...
	public ContainerWoodPlanter(EntityPlayer pl, TileEntity te, World wd, int x, int y, int z){
		...
		seedInventory = (TileEntityWoodPlanter)te;
		world = wd;
		xCoord = x;
		yCoord = y;
		zCoord = z;
	...
	public void setDeliverMode(Boolean deliver) {
		((TileEntityWoodPlanter)seedInventory).setField(1, BooleanUtils.toInteger(deliver));
	}

	public World getWorld() {
		return ((TileEntityWoodPlanter)seedInventory).getWorld();
	}
}